...
function modifyValidatorEscape(_validator, _oldValidatorEscapeRef) {
  _validator.escape = function escape(obj, response = false) {
    ...
    function _safeEscape(_str, escapeFunction, unescapeFunction) {
      // string value may have already been escaped, which may cause other literals like `<` to be re-escaped,
      // for example, `<` turns into `&lt;` which if re-escaped will turn into `&amp;lt`
    ...
    if (response === true && _isString(obj)) {
      try {
        const tmp = JSON.parse(obj);
        obj = tmp;
      } catch (e) {
        console.error('validatorUtility', e);
      }
    }
